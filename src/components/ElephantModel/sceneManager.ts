...
// 🚀 MODIFICATION 1: Expand the return type to include exposed scene methods
export type SceneSubjectReturn = void | { 
    dispose?: () => void;
    snapPage?: () => void;
    animateOut?: (duration: number) => Promise<void>; // <-- NEW: Expose animateOut
    animateIn?: () => void; // <-- ADDED: Assumed reset/re-init function
}; 
...
export function SceneManager(canvas: HTMLElement, initialSubject: SceneSubject, gui: any) {
    ...
    // 🚀 MODIFICATION 2: Store the subject's exposed methods here.
    let currentSubjectMethods: {
        snapPage?: () => void;
    ...
    const applySubject = async (subject: SceneSubject) => {
        ...
        if (currentCleanup) {
            ...
            currentCleanup = null;
            currentSubjectMethods = null; // Clear old methods
        }
        
        // run new subject
        const ret = await subject(canvas, scene, camera, renderer, gui);
        
        // 🚀 MODIFICATION 3: Store exposed methods if they exist.
        if (ret && typeof ret === 'object') {
            if (typeof ret.dispose === 'function') {
                currentCleanup = ret.dispose;
            }
            // Store the exposed methods for external access
            currentSubjectMethods = {
                snapPage: ret.snapPage,
                animateOut: ret.animateOut,
                animateIn: ret.animateIn,
            };
        }
    };
    ...
    return {
        ...
        // 🚀 MODIFICATION 4: Expose a getter for the current subject's methods
        getCurrentSubjectMethods: () => currentSubjectMethods,
    };
}
...
export function getSceneManager(container: HTMLElement, sceneSubject: SceneSubject, gui: any) {
